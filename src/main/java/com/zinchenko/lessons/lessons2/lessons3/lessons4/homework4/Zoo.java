...
public class Zoo {
    ...
    public static void main(String[] args) {
        ...
        int bears = getRandomNumber(max, min);
        int tigers = getRandomNumber(max, min);
        int parrots = getRandomNumber(max, min);
        int elephants = getRandomNumber(max, min);
        int raccoons = getRandomNumber(max, min);
        int lions = getRandomNumber(max, min);
    ...
    //Создаем метод вывода от 2х до 30
    ...
    // сделали перегрузку методов бчтоб не менять вложенные переменные добавили еще один метод
    public static int countAnimals(int bears, int tigers, int parrots, int elephants, int raccoons, int lions) {
        // int result = countAnimals(bears, tigers, parrots, elephants, raccoons)+ lions;//чтоб не прописывать в ручную скопировали метод
        return countAnimals(bears, tigers, parrots, elephants, raccoons) + lions;//можем и так вывести
    }

    public static int countAnimalsNextYear(int bears, int tigers, int parrots, int elephants, int raccoons) {
        // System.out.println((int) (bears + COEFFICIENT_BEARS));
        //  System.out.println(bears * COEFFICIENT_BEARS);
